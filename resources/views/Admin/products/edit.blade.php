                            ...
                            <form action="{{route('product.update',$product)}}" method="post" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea required name="description" rows="7"
                                              ...
                                              id="description"
                                              placeholder="الوصف ">{{$product->description}}</textarea>
                                    @error('description')
                                    <div style="margin-top: 2px" class="alert alert-danger">
                                        {{$message}}
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" step=".000001" required name="price"
                                           ...
                                           placeholder="السعر" value="{{$product->price}}">
                                    @error('price')
                                    <div style="margin-top: 2px" class="alert alert-danger">
                                        {{$message}}
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="store_id" class="form-control select2" style="width: 100%;">
                                        @foreach(\Appxz\Models\Store::all() as $store)
                                            <option
                                                value="{{$store->id}}" {{$store->id==$product->store->id ? 'selected': ''}}>{{$store->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
